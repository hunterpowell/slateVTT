...
//   grid    map squares. What a token's position is *stored* in (invariant 1).
//
// Device pixel ratio is absent here. It is folded into the canvas transform in
// render.ts and nowhere else, so `screen` is always CSS pixels.
 ...
 *
 * The two axes define the shape. A square grid is `(px, 0)` and `(0, px)`; an
 * isometric one is a diamond's two half-diagonals. Everything downstream places
 * things through `gridToWorld`, so only this file and `gridBasis` in `scene.ts`
 * know there is more than one kind of lattice.
 *
 * `px` is stored, not derived at each use, and it is the server's `grid_px`:
 * **the size of a cell, for sizing things that stand on the grid, never for
 * placing them.** A token's diameter and the zoom at which the grid's halo is
 * worth drawing are what it is for. Anything that computes a *position* from it
 * is assuming squares.
 */
 ...
 *
 * `null` means the whole image. The server can't say it any other way, since it
 * never sees the image. The clip matters for speed: the renderer rules one grid
 * line per cell across this rectangle, so a stale or oversized one from a save
 * would otherwise cost frames.
 */
...
/**
 * A square grid, the default. Named so that a square lattice is stated in the
 * code instead of being four numbers a reader has to recognise.
 */
...
export function worldToGrid(grid: GridSpec, wx: number, wy: number): Vec2 {
  ...
  // from zero and so is an isometric ratio. Returning the origin instead of
  // `NaN` keeps a degenerate grid a visible bug, where `NaN` would spread
 ...
 * sheared into the right parallelogram by the matrix. The fog, the override
 * paint, the shape fills and the ruler's trail draw cells this way and need no
 * isometric case. A per-cell bitmap `drawImage`d under it lands in place for the
 * same reason.
 *
 ...
 * so mixing them up only shows on an isometric map.
 */
 ...
 *
 * `px_per_cell` in `server/src/fog.rs` is the same function, and both exist
 * for the same caller: a wall further from the eye than `n` cells cannot be
 * crossed by a ray `n` cells long, whichever direction it is cast, so this is
 * what a once-per-source wall cull bounds itself by. On a square grid it is
 * `px`.
 *
 * The largest singular value of the basis. Don't use `|u| + |v|`: it is also
 * safe but looser, and on a square grid it would be `2 * px`, halving the
 * culling on every map.
 */
export function maxSpan(grid: GridSpec): number {
  ...
  // Zero for a square grid and non-negative for any basis; the clamp absorbs
  // float error, not a real case.
 ...
 *
 * Not rounded here, because its two callers round differently and making them
 * share a rounding is a bug. Ruling the grid wants the whole-numbered *lines*
 * inside the rectangle, so it rounds inward: `ceil` the low end, `floor` the
 * high. Sweeping cells wants every cell the rectangle
...
// Snapping to the nearest cell centre does NOT live here. The server applies
// it on the drop frame and echoes the settled position back;
