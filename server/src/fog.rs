//! What the party can see, and what they have seen. This module is what reads
//! the walls.
//!
//! Two sets of cells, both party-shared: `visible` is current line of sight,
//! `revealed` is everywhere the party has ever had it. Terrain gates on `known`
//! (`revealed` widened by `with_fringe`) and tokens gate on `visible`. The room
//! applies those gates; this module only answers which cells are which.
//!
//! **Read `docs/fog.md` before changing anything here.** The coordinates are the
//! trap: a cell is grid units, a wall is image pixels, and every ray in this file
//! is cast in image pixels because that is the space the walls live in.
//!
//! The exception is the radius, which is set in feet and so is a whole number of
//! cells. The sweeps take their sources in grid units and measure the reach
//! there, because the cells exactly on the circle are a tie, and scaling that tie
//! into pixels answers it differently on each side. See *The radius is measured
//! in cells* in `docs/fog.md`.
...
///
/// A tuple, unlike `Pos` and `Px`: it is an index into a lattice, not a
/// position on the board, and as a `HashSet` key a tuple needs one derive
/// instead of four.
///
/// **Outbound it is never serialized as itself, but inbound it is.** `FogView`
/// packs a whole rectangle of cells into one string. `SetFogOverride` carries
...
/// Don't assume the packing holds in both directions: that assumption made
/// `MAX_OVERRIDE_CELLS` 25× what a frame could hold.
...
///
/// The client has the same constant in `ruler.ts`. It isn't sent on the wire
/// because it's a fact about the game, not about this room, and a frame carrying
/// it could disagree with itself.
pub const FEET_PER_CELL: f32 = 5.0;

/// How far a vision radius may be set, in feet. The cell sweep below is
/// quadratic in the radius, and on a map with no play area to clip against, the
/// ceiling is the only thing bounding the loop.
pub const MIN_VISION_FT: f32 = 5.0;
pub const MAX_VISION_FT: f32 = 500.0;

/// One thing that lights the board, and how far it reaches.
///
/// Grid units like `Pos`, unlike the walls, for the reason `visible_cells`
/// gives: the radius is compared in cells because the four cells at exactly
/// that distance are a tie, and scaling a tie into pixels answers it
/// differently on each side.
///
/// **`radius_ft` is `None` for "as far as this map lets anyone see".** The
/// fallback lives here, not at the call sites, so `room.rs` can pass
/// `token.light_ft` unchanged for a party member and for a brazier alike. The
/// `?? vision_ft` rule exists once, in `radius_cells`.
...
///
/// A defensive limit nobody should meet. The box is the bounding box of
/// explored terrain, which is clipped to the board, so reaching this needs the
/// smallest legal grid on the largest legal image. That configuration already
/// costs the client hundreds of thousands of grid lines a frame (see
/// `MIN_GRID_PX`). Past the limit the fog packs as nothing, which the client
/// draws as a dark board: a visible failure on one map, not an allocation that
/// takes the room down for everyone.
const MAX_FOG_SIDE: u32 = 4096;

/// Never seen. The client draws this solid.
const DARK: char = '#';
/// Seen once, not in sight now: explored terrain. Drawn dim, because the map is
/// remembered and the creatures standing on it are not.
const KNOWN: char = 'o';
/// In sight right now. Drawn clear.
const SEEN: char = '.';

/// No override on this cell, so sight decides it. The gap in an `OverrideView`,
/// and the only one of the four that is never stored: `Auto` is the absence of
/// an entry in the map, so "not overridden" has one representation, not two
/// that could disagree.
const AUTO: char = '-';
...
/// Forced dark, the only override that takes something away.
const FORCED_DARK: char = '#';

/// A rectangle of cells packed one character each, row-major. Not a JSON array
/// of per-cell values: the wire protocol asks for frames a human can read in
/// devtools, and a few thousand numbers isn't one. A few thousand characters laid
/// out as a map is, because the shape of the dungeon shows in the string.
///
/// Two things are packed this way, and they have separate names below because a
/// character means something different in each and they go to opposite
/// audiences. The encoding lives here; which alphabet a rectangle uses is a fact
/// about the field carrying it.
...
/// which lets the box shrink to the explored part instead of covering the image.
///
/// `None` in place of one of these means the map is not fogged. Like `staged`
/// being `None`, that is the only thing the server could mean by it.
///
/// The same value reaches every recipient, unlike `OverrideView` below: the wall
/// geometry is the secret, and this is only the shadow.
pub type FogView = CellRect;

/// The DM's manual override, packed the same way and sent to the DM alone.
///
...
/// painted on packs to nothing. An empty one therefore means both "nothing
/// overridden" and "you are not the DM", as an empty wall list does.
///
/// Don't send a `Vec<(Cell, Override)>`: a flood fill is thousands of cells, and
/// thousands of JSON pairs is the frame `FogView` exists to avoid.
pub type OverrideView = CellRect;

/// What the DM has said about a cell, overriding what the rays say.
///
/// Three variants, not four: `Auto` is the absence of an entry, so a cell set
/// back to it is removed, not stored. Kept apart from line of sight because
/// anything written into `revealed` instead would be lost the next time a torch
/// reached the cell.
///
/// **`Explored` and `Lit` are floors and `Dark` is a ceiling**, which makes
/// applying them in `recompute_sight` order-independent and total. `Explored`
/// does not demote a cell the rays already lit; to show the room but not the
/// ambush in it, use `hidden`.
...
/// three states describe every case and no cell needs two fields.
...
///
/// Both lit states count as revealed, so this is the inverse of `pack`'s first
/// question and doesn't care which of the two the file recorded. `visible` is
/// not recovered: it is derived from where the tokens stand, which the same file
/// holds, and deriving it means a save written before a wall moved can't
/// describe sight through it.
...
/// The overrides back out, for a room coming off disk. Unlike the fog, this is
/// restored whole: it is what the DM said, not what the rays found, and nothing
/// in the room can derive it.
...
///
/// A `w` that disagrees with the string means a damaged file, not a schema
/// change, and `chars().nth()` per cell would be quadratic. Walking the
/// characters in order and deriving the row handles both.
...
/// Every cell at least one of `sources` has line of sight on. This is
/// `Lighting::Dynamic`.
///
/// Raycasting to cell centres, not shadowcasting. Shadowcasting needs
...
/// into blocking cells changes the dungeon: a wall traced along a cell boundary,
/// the common case, would blind the cells on both sides of the line and shrink
/// every room it encloses.
///
/// The rule is one sentence: a cell is visible when the straight line from the
/// viewer's centre to that cell's centre crosses no solid wall and no shut door.
/// It reads the segments as they were traced, a player who asks why they can't
/// see something can follow it, and it is a page of code, not three.
///
...
/// cell and "within 20 ft" is a square. `docs/drawings.md` names the same
/// disagreement and leaves it standing.
///
/// **The radius is measured in cells, which is why `sources` is in grid units
/// while every other coordinate in this file is pixels.** A radius set in feet
/// is a whole number of cells, so the four cells at exactly that distance sit on
/// the circle: a tie, like the wall-corner ones `crosses` settles. Measured in
/// pixels, the two sides of that tie round differently and the circle loses a
/// cell off one edge and keeps it on the other. In cells the arithmetic is exact
/// and both sides agree. See *A tie is answered the same way from both ends* in
/// `docs/fog.md`.
///
/// Cost is cells swept times walls not culled, per source. Both are bounded
/// before the loop: walls further from the viewer than the radius can't be
/// crossed by any of its rays and are dropped once per source, not once per
/// cell, and the sweep is clipped to the play area.
pub fn visible_cells(map: &MapInfo, walls: &[Wall], sources: &[Source]) -> HashSet<Cell> {
    ...
    for src in sources {
        // Each source has its own reach, so this is inside the loop. A source
        // naming no radius takes the map's (see `Source`).
        let eye = src.at;
        ...
        for dy in -reach..=reach {
            for dx in -reach..=reach {
                ...
                // The play-area boundary is in `blockers` as well, which stops a
                // viewer standing off the board seeing onto it. This covers the
                // other direction: the void beyond the edge is not somewhere the
                // party explores, whatever the geometry says.
                //
                // With no play area the check is still needed. A token dragged
                // to cell one million would otherwise reveal cells there, and the
                // rectangle packing them alongside the dungeon would be the whole
                // map's worth of characters on every send. `MAX_MAP_PX` is where
                // the walls stop too.
...
/// ```
///
/// **`Room` is a union, not a replacement.** In one sentence for the DM: *you
/// see the whole room you are standing in, plus whatever you have a straight
/// line to.* It never shows less than `Dynamic` would, and it is what lets a
/// doorway carry sight but not light: `lit_cells` stops at every segment the DM
/// traced, open or shut.
...
///
/// The gate on a light nobody is carrying. A brazier the DM placed while
/// preparing a dungeon must not light a room the party can't see, or promoting
/// that map hands over every lit chamber on the level at once. `visible` is a
/// flat union of its sources with no "and somebody can see it" term, so the term
/// has to be applied to the source list before the sweep runs.
///
/// **This is not `visible_cells` narrowed to one cell.** It asks about the
/// geometry and never about a radius: with vision set at thirty feet, a brazier
/// at forty is one the party can plainly see, so gating on their reach would be
/// wrong. That is why there is no wall cull here (a cull is bounded by a radius,
/// and there is none). It also keeps this cheap: a segment test per eye, where
/// gating on `visible` would be a second sweep.
///
/// It uses the same blockers the rays stop at, so an open door passes this as it
/// passes sight, and `crosses`'s permissive end ties apply: a light traced onto a
/// wall's corner is lit, not dark against its own masonry.
...
/// the DM traced, out to the radius. This is half of `Lighting::Room`; the other
/// half is the rays `sight_cells` unions in.
///
/// A cell is lit when a four-neighbour walk reaches it, so a room lights as a
/// room: the party steps into a chamber and sees all of it, not the wedge their
/// torches point at.
///
/// **Every traced segment bounds it, open or shut, which is `fillFrom`'s rule in
/// `overrides.ts`, not the raycast's.** Don't read `blocks()` here: that makes an
/// open door a hole in the room boundary, so a one-cell hallway hands over the
/// whole chamber past it, and the only thing that bounds a room is a shut door
/// the DM has to swing by hand. An archway is traced as a door left open, the
/// same way the DM already traces one for the reveal tool.
///
/// An open door passes sight instead: the rays run alongside this and stop at
/// `blocks()`, so the party gets the wedge they can see through the doorway, not
/// the room behind it. Only sight reads a door's state.
///
/// So this asks the same question `fillFrom` asks, in a second language. The
/// duplication stays for `shape_covers`'s reason: that one previews what the DM
/// is about to paint, this one decides what the party is sent, and a
/// disagreement at the fringe changes a preview, not a permission.
///
/// The radius bounds it as well as the walls. A pure fill ignores corners: walk
/// into a winding corridor and all of it lights to its far end, around every
/// bend. The radius is Euclidean from the source, like the raycast's, so the
/// number means the same thing in both modes. A hall bigger than the radius is a
/// map whose radius should be raised, not a reason for a second number.
///
/// One fill per source, unioned, not one sweep sharing a visited set. The
/// raycast may skip a cell another torch already lit because its rays are
/// independent. Here, skipping such a cell would stop this source expanding
/// through it, and a fill that never enters the corridor never reaches the room
/// past it.
pub fn lit_cells(map: &MapInfo, walls: &[Wall], sources: &[Source]) -> HashSet<Cell> {
    ...
    // Every segment, unlike the raycast's list in `visible_cells`: a door bounds
    // a room whatever it is swung to, and an open one passes only sight.
    let blockers: Vec<(Px, Px)> = walls
        .iter()
        .map(|wall| (wall.from, wall.to))
        .chain(boundary(map.play_area))
    ...
    for src in sources {
        // In cells for the step test and in pixels for the wall cull, the same
        // split as the raycast and for the same reason. Per source, as the
        // raycast takes it.
        let eye = src.at;
        let radius_cells = src.radius_cells(map);
        let radius = radius_cells * px_per_cell(map);
        ...
        while let Some(cell) = queue.pop() {
            ...
            // and expanded out of by none. This matches `cutByWall` in
            // `overrides.ts` segment for segment. Corner-snapped masonry can't
            // produce such a cell, but a wall at 45 degrees hits a centre every
            // other cell, and a chamfered room corner is made of those. Without
            // this the fill walks in one side and out of the other, and one such
            // cell hands the table the whole map.
            //
            // The seed is not special-cased, for the same reason as in
            // `cutByWall`: letting it expand would put both sides of the wall in
            // one fill. A token standing inside masonry lights only its own
            // square, which is visibly wrong and the DM will fix it.
            ...
            for (dx, dy) in [(1, 0), (-1, 0), (0, 1), (0, -1)] {
                ...
                // The same two bounds the raycast takes, for the same reasons:
                // the void off the edge is not somewhere the party explores, and
                // a cell out there would stay in the packed rectangle.
...
///
/// Asked of everything in `near`, doors included whatever they are swung to. That
/// is the list the steps are tested against, so the geometry that stops a step
/// also decides which cells the fill may not leave.
...
///
/// This is for the masonry. `snapToCorner` puts a traced wall on the corner
/// lattice, so it runs between cell centres and the last cell a ray reaches is
/// the floor square inside the room. If the DM traced along the inner face of
/// the wall (the natural way to trace one), the drawn wall is past that cell and
/// the table is shown floor, then nothing. Rooms read as holes. One cell of
/// fringe puts the wall on their board.
///
/// It is a set operation and knows nothing about walls, so the fringe lands in
/// every direction, not only across masonry: one cell further down an open
/// corridor as well, which makes terrain `vision_ft` plus a square. That reads
/// as the corridor ahead fading out. Asking the raycast which cells it was
/// blocked into instead would need a different return type for a picture nobody
/// could tell apart.
///
/// **Only `known` is built through this.** Never `visible`, which gates the
/// creatures, and never `revealed`, which is memory and written by rays only.
/// The output always contains the input: each cell goes in before the board is
/// checked, so `visible ⊆ known` can't depend on where some other bound
/// clipped. The ring is clipped, because the void off the edge of the map is not
/// somewhere the party explores, and a fringe there would be a cell in the
/// packed rectangle.
///
/// Eight neighbours, not four: a four-neighbour ring leaves a notch in every room
/// corner, which is more visible than the problem this fixes.
...
///
/// This is the implicit wall. No editor produces one because it is already on
/// `MapInfo`, and a DM shouldn't have to trace the edge of their own map.
...
///
/// The play area when there is one. When there isn't (the DM's "whole map"
/// button leaves none, so this is the common case), the bound is `MAX_MAP_PX`,
/// the largest anything in this coordinate space may be.
...
///
/// The same bound the sweeps clip to, for the same reason: an override on cell
/// one million puts that cell in the bounding box of `pack_overrides`, and the
/// rectangle spanning it and the dungeon is the whole map's worth of characters
/// on every send. A client bug is enough to do it.
...
///
/// **The one place `GridShape` is read on the server.** An isometric grid is an
/// affine transform of a square one, so every function below works on a lattice
/// and none of them knows there is more than one shape, as `sight_cells` is the
/// only reader of `Lighting`. `gridBasis` in `client/src/scene.ts` does the same
/// on the client and must agree with this.
///
/// The square case is `((grid_px, 0), (0, grid_px))`, which reduces the three
/// conversions below to plain scaling by `grid_px`. That is why a board saved
/// as squares is unaffected by the basis.
fn basis(map: &MapInfo) -> (Px, Px) {
    match map.grid_shape {
        ...
        // run to its right-hand and left-hand corners. Mirrored about the
        // vertical, so one dragged edge is enough to calibrate one. See
        // `docs/maps.md`.
        GridShape::Iso { ratio } => {
...
///
/// The largest singular value of the basis, which is the number the wall culls
/// need: a ray of `n` cells can't travel further than `n * scale` pixels in any
/// direction. On a square grid it is `grid_px`.
///
/// Not `|u| + |v|`, which is also safe but looser: on a square grid that would
/// be `2 * grid_px`, and the culls would keep every wall within twice the
/// radius on every map.
fn px_per_cell(map: &MapInfo) -> f32 {
    ...
    // basis; `max(0.0)` only absorbs float error.
...
/// The middle of a cell, in image pixels. Every ray in this file points here.
...
///
/// Touching is the difficulty here, and it is common: cell centres and
/// `snapToCorner`'d wall corners sit on the same lattice, so a ray passing
/// exactly through a wall's endpoint happens all the time. The rule is **ties at
/// the ray's own ends are permissive, ties in the middle of it are not**, and it
/// decides three cases:
///
/// - A viewer standing on a wall is not blinded by it. Otherwise a creature in a
///   doorway sees nothing. That is a tie at `p`.
/// - A ray ending exactly on a wall's endpoint slips past its tip. This is
///   corner peeking, which every VTT has, and it errs towards showing the player
///   something. That is a tie at `q`.
/// - A wall's corner lying exactly on the ray stops it. That is a tie in the
///   middle, and it must give the same answer whichever end the ray is cast
///   from. Negating a side flips a negative to positive but leaves a zero
///   reading as "not positive", so a naive test blocks sight one way along the
///   line and passes it the other. A wall that stops a ray has to stop it
///   whoever is looking.
fn crosses(p: Px, q: Px, a: Px, b: Px) -> bool {
    ...
    // it, and that corner can't be `p` or `q` (either would have returned
    // above), so if it is between them at all it is strictly between them.
    let d3 = cross(p, q, a);
    let d4 = cross(p, q, b);
    if d3 == 0.0 {
        return within(p, q, a);
...
/// between them. The points are collinear, so a bounding-box check is enough and
/// there is no length to divide by.
...
///
/// A monster is visible when any cell it covers is. A four-cell ogre leaning
/// into a lit corridor is one the party can see, and asking only about its
/// centre would hide it behind the wall it is standing beside.
///
/// Anything smaller than a cell covers the cell it is in, matching
/// `snap_to_cell`: a druid who is currently a rat is standing in one square, not
/// a quarter of one.
pub fn covered_cells(x: f32, y: f32, size: f32) -> Vec<Cell> {
    let half = size.max(1.0) / 2.0;
    // A token's edges land exactly on grid lines, so `floor` is deciding an
    // exact tie at both ends. The nudge keeps a 2×2 at (4.0, 4.0) covering
    // cells 3 and 4, not 3, 4 and 5.
    let slack = 1e-4;
...
/// makes a wedge as wide at its far end as it is long. `shapes.ts` has the same
/// constant; see `shape_covers` for why the rule is written twice.
...
/// cells. Matches `COVERAGE_SLACK` in `shapes.ts`, for the reason given there: a
/// cone is narrow near its apex, and without slack the squares plainly in front
/// of it fall out.
...
/// fog overlay would put it on the client, which invariant 4 forbids.
///
/// This is the same rule `coveredCells` applies in `shapes.ts`, written a second
/// time in a second language. The filter has to run where the decision is made,
/// and the client has to run it to tint the cells it draws. The two only have to
/// agree loosely: a disagreement at the fringe changes whether a frame is sent,
/// never how it draws, because the client draws what it is given.
///
/// Walks the shape's bounding box, not `cells`, so the cost is proportional to
...
///
/// Sampled every half cell, not walked with Bresenham: the step can't skip a
/// cell it passes through, the count is bounded by the same cap as the areas,
/// and it is four lines instead of twenty. Duplicates are fine, because the
/// caller asks whether any of them is in a set, not how many.
...
///
/// The port of `containsPoint` in `shapes.ts`, including the cone test. The cone
/// is resolved along the wedge's own axis, not into an angle, so a cone pointing
/// due west can't hit the discontinuity in `atan2`.
fn contains_point(kind: ShapeKind, origin: Pos, to: Pos, px: f32, py: f32) -> bool {
    ...
    match kind {
        ...
        ShapeKind::Cone => {
            ...
            // The wedge as a perpendicular distance to its edge, not an angle,
            // so it can be compared with a slack measured in cells, and the test
            // is the same at the apex as at the tip.
...
mod tests {
    ...
    fn the_rectangle_is_the_bounding_box_of_what_is_explored() {
        ...
        // definition, which lets the box shrink to the explored part.
    ...
    fn the_three_states_pack_one_character_each_row_major() {
        ...
        // order), so one character describes every case.
    ...
    fn both_lit_states_unpack_as_explored() {
        ...
        // `unpack` has to read either without being told which it has.
    ...
    fn a_square_basis_is_the_arithmetic_it_replaced() {
        // A square basis must reduce to `offset + n * grid_px`. If it doesn't,
        // every token on every saved map is somewhere new.
        let map = MapInfo {
            grid_px: 70.0,
            offset_x: 3.0,
            offset_y: -4.0,
    ...
    fn a_point_and_the_cell_it_falls_in_agree_on_an_isometric_grid() {
        // `cell_of` is `grid_to_px` inverted and floored, and the raycast uses
        // the pair twice per ray. An inverse that is slightly wrong shows up as
        // fog one cell out, which is hard to see and easy to ship.
    ...
    fn nothing_reaches_further_than_a_cell_can() {
        // `px_per_cell` bounds the wall culls, so it has to be at least the
        // longest distance in pixels that one unit of grid distance can span.
        // The candidates are the diamond's two diagonals, and the wider one wins.
        let map = iso_map(64.0, 2.0);
    ...
    fn sight_reaches_exactly_as_far_as_the_radius_and_is_a_circle() {
        // Euclidean, so a radius of light is a circle. That agrees with a drawn
        // circle and disagrees with the movement ruler, where a diagonal step
        // costs one cell. `docs/drawings.md` names that disagreement.
    ...
    fn the_circle_is_the_same_on_both_sides_of_the_viewer() {
        ...
        // exactly that distance sit on the circle: due east, due west, and (at
        // twenty-five feet) the two ends of every 3-4-5 triangle. That is a tie,
        // and as in `crosses`, a tie must get the same answer from either side.
        //
        // Don't measure it in pixels. `(c + 0.5) * grid_px - x * grid_px` and
        // `radius_cells * grid_px` are two roundings of the same number that
        // disagree in the last bit, so the cell six east lands inside the circle
        // and the cell six west outside it: a lopsided circle that changes shape
        // as the token walks. A power-of-two grid hides this, so the grids here
        // are the awkward ones a DM's map calibrates to.
    ...
    fn a_viewer_standing_on_a_wall_is_not_blinded_by_it() {
        ...
        //
        // Their own square is not enough to assert. It can pass while the wall
        // still blinds them westward and not eastward, because a zero side
        // reads as "not positive" and negating the other side flips it.
    ...
    fn a_wall_corner_on_the_ray_stops_it_whichever_end_is_looking() {
        // Cell centres and snapped wall corners are on the same lattice, so a
        // ray straight through a wall's endpoint is the common case. The answer
        // must not depend on the direction of travel: if it does, one of these
        // two sees the other and is not seen back.
    ...
    fn a_closed_room_contains_its_own_light_and_one_missing_cell_does_not() {
        // Masonry alone encloses a room, with no door needed anywhere. The
        // other wall tests imply this without stating it.
        //
        // The second half matters more. A gap of a single cell (an untraced
        // doorway, or a polyline that never returned to its first corner) leaks
        // the whole map. Nothing about it is gradual, which is why a room that
        // looks traced can behave as though it weren't, and why the fill
        ...
        // A shut door hung in the gap closes it.
        let mut shut = gap;
        shut.push(Wall {
    ...
    ///
    /// `door` hangs one in the middle of the south wall; `None` makes that wall
    /// solid instead. Every test below says which it wants, because half of them
    /// are about that difference.
    ...
    fn an_open_door_bounds_the_flood_like_everything_else_traced() {
        // The same rule as the DM's fill. Reading `blocks()` here would make an
        // open door a hole in the room boundary, so a one-cell hallway would
        // hand over the whole chamber past it, and the only thing that bounded
        // a room would be a shut door somebody had to swing by hand.
        //
        // The source stands in the doorway's own column, the case that leaks
        // most if the rule is wrong.
    ...
    fn what_an_open_door_passes_is_sight() {
        // The other half of the union, and why the flood can stop at the
        // doorway: the party gets the wedge they can see through it, not the
        // room behind it. Straight down the door's own column reaches; a cell
        // whose ray hits the masonry beside it does not.
    ...
    fn a_shut_door_passes_neither() {
        // Both halves stop, which is what "a shut door seals a room" means: the
        // flood bounds on it like any segment, and the rays stop because it
        // blocks.
    ...
    fn the_fill_is_bounded_by_the_radius_as_well_as_by_the_walls() {
        // Without this a pure fill ignores corners: one step into a winding
        // corridor lights all of it, around every bend. The radius is Euclidean
        // like the raycast's, so the number means the same thing in both modes.
    ...
    fn a_wall_the_fill_cannot_cross_is_not_one_it_can_walk_along() {
        // The other half of the previous test's bound: inside the radius, the
        // masonry stops the fill. A room bigger than the radius is a map whose
        // radius should be raised, not a reason for a second number.
    ...
    fn a_cell_a_wall_runs_through_is_a_dead_end() {
        ...
        // the other. Without the dead-end rule the fill walks in one side and
        // straight out of the other, and one such cell leaks the whole map.
        //
        // The chamfer is taken by the fill that reached it, which keeps a
        // room's own corner from coming back ragged.
    ...
    fn two_torches_light_two_rooms_and_neither_lights_the_others() {
        // One fill per source, unioned, not one sweep sharing a visited set,
        // which would stop the second source expanding through a cell the
    ...
    fn the_mode_is_the_maps_and_a_room_is_never_less_than_sight() {
        // `sight_cells` is the one place the mode is read, which keeps
        // `recompute_sight` a single call. `Room` is a union, so it never hands
        // the table less than line of sight already would.
        // A spur inside the chamber gives the flood somewhere the rays can't
        // reach. In a bare room the two agree exactly, which proves nothing.
        let mut walls = chamber(None);
    ...
    fn a_room_lit_map_reaches_a_corner_the_rays_cannot() {
        // What the mode is for, stated as a difference, not a count: an
        // L-shaped alcove off the chamber is lit whole under `Room` and only in
        // the wedge the torch points at under `Dynamic`.
        let mut walls = chamber(None);
        walls.push(masonry(320.0, 128.0, 320.0, 384.0));
    ...
    fn the_three_states_pack_in_their_own_alphabet() {
        // A different alphabet from the fog's, and it has to be: `o` and `#`
        // mean the same thing in both, but the fog has no `*`, and `-` marks a
        // gap, which the fog never has.
    ...
    fn the_overrides_survive_the_round_trip_including_the_holes() {
        // A cell nobody painted has to come back as an absent entry, not as a
        // fourth variant, because absence is how the room stores `Auto`.
        let painted = HashMap::from([
            ((-3, -2), Override::Lit),
            ((4, 5), Override::Dark),
    ...
    fn a_circle_covers_the_cells_a_circle_covers() {
        ...
        // tints on the client, which is why it was ported and not approximated
    ...
    fn a_cone_is_a_wedge_and_its_apex_is_inside_it() {
        // The apex is easy to get wrong (there is no angle from a point to
        // itself), so it is asserted here too.
        let o = (2.5, 2.5);
        let to = (6.0, 0.0);
